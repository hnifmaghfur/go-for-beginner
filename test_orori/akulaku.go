...
func generateSignature (content string) string{
	...
	//signature := base64.RawStdEncoding.EncodeToString([]byte(security.Sha512Encrypt(content)))
	sha512 := security.Sha512Encrypt(content)
	sha512hex,_ := hex.DecodeString(sha512)
	signature := base64.URLEncoding.EncodeToString(sha512hex)
	return signature
}